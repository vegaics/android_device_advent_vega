...
int hostapd_reload_config(struct hostapd_iface *iface)
{
     ...
     * response frames) */
    if (hostapd_set_ssid(hapd, (u8 *)hapd->conf->ssid.ssid,
     ...
     if ((hapd->conf->ieee802_1x || hapd->conf->wpa) &&
        ...
        wpa_printf(MSG_ERROR, "Could not set IEEE 802.1x for kernel driver");
        return -1;
    }
...
static void hostapd_cleanup(struct hostapd_data *hapd)
{
    ...
    /* Set ESSID off for the kernel driver) */
    if (hostapd_set_ssid(hapd, NULL, 0)) {
        wpa_printf(MSG_ERROR, "Could not set ESSID  off for kernel driver");
    }
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
    ...
    /* Set SSID for the kernel driver (to be used in beacon and probe
     * response frames) */
    if (hostapd_set_ssid(hapd, (u8 *) conf->ssid.ssid,
                     conf->ssid.ssid_len)) {
        wpa_printf(MSG_ERROR, "Could not set SSID for kernel driver");
        return -1;
    }
    ...
    if (conf->radius_server_clients &&
        ...
        return -1;
    if (hostapd_set_freq(hapd, hapd->iconf->hw_mode, conf->channel_num, 0, 0)) {
