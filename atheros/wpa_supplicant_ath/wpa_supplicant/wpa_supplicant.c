 ...
 * attempt.
 */
void __wpa_supplicant_req_auth_timeout(struct wpa_supplicant *wpa_s,
				     int sec, int usec, const char *func)
{
	if (wpa_s->conf && wpa_s->conf->ap_scan == 0 &&
	    wpa_s->driver && IS_WIRED(wpa_s->driver))
		return;
	wpa_msg(wpa_s, MSG_INFO, "Setting authentication timeout: %d sec "
		"%d usec %s", sec, usec, func);
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (bss_rsn && (ssid->proto & WPA_PROTO_RSN) &&
		...
		proto = WPA_PROTO_WPA;
#ifdef CONFIG_WAPI
	} else if ((ssid->proto & WPA_PROTO_WAPI) && bss && wpa_scan_get_ie(bss, WLAN_EID_WAPI)) {
...
wpa_msg(wpa_s, MSG_DEBUG, "WPA: GTK: %x %x %x", ie.group_cipher, ssid->group_cipher, ie.group_cipher & ssid->group_cipher);
...
#ifdef CONFIG_WAPI
	/* Clear WAPI status */
	wpa_drv_set_wapi(wpa_s, (ssid->proto & WPA_PROTO_WAPI) );
#endif
